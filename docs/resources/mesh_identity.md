...
- `type` (String) Type specifies the type of certificate provider. possible known values include one of ["Bundled", "Spire", "Extension"]
...
- `extension` (Attributes) Extension indicates that custom provider is used. (see [below for nested schema](#nestedatt--spec--provider--extension))
...
<a id="nestedatt--spec--provider--extension"></a>
### Nested Schema for `spec.provider.extension`

Required:

- `name` (String) Name is the name of the extension provider.

Optional:

- `config` (String) Config is a freeform configuration for the extension. Parsed as JSON.
